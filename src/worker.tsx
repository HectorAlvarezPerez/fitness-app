...
app.get('/api/pr', async (c) => {
  ...
  const [historyResult, exerciseResult, persistedResult] = await Promise.all([
    ...
    client.from('personal_records').select('exercise_name,weight,reps,date').eq('user_id', userId),
  ]);
...
app.patch('/api/exercises/:id', async (c) => {
  ...
  if (typeof body?.name === 'string') {
    const { data: existingNames, error: namesError } = await client
      .from('exercises')
      .select('id,name');
  ...
  if (Array.isArray(body?.secondary_muscles)) {
    updates.secondary_muscles = body.secondary_muscles.filter(
